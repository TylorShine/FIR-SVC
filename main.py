...
def parse_args(args=None, namespace=None):
    ...
    parser.add_argument(
        ...
        # default=5292000,
        default=None,
        help="num of frames for each segment | default: None",
        # 5292000 (2 minutes with fs==44100)
    )
    return parser.parse_args(args=args, namespace=namespace)

    
def split(audio, sample_rate, hop_size, db_thresh = -40, min_len = 5000, frames=None):
    slicer = Slicer(
                sr=sample_rate,
                threshold=db_thresh,
                min_length=min_len)       
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for segment in tqdm(segments):
            ...
            # print(start_frame, start_frame + seg_units.size(1), seg_units.size(1))
            ...
            if padded_length > 0:
                # print('Padded length:', padded_length, 'seg_output', seg_output.shape)
                seg_output = seg_output[:, :-padded_length]
                # print('After trim:', seg_output.shape)
                padded_length = 0
                
            # print(seg_input.shape, seg_units.shape, seg_output.shape, (start_frame + seg_units.size(1)) * args.data.block_size - start_frame * args.data.block_size)
            # print(mask[:, start_frame * args.data.block_size : (start_frame + seg_units.size(1)) * args.data.block_size].shape)
            
            # seg_output *= mask[:, start_frame * args.data.block_size : (start_frame + seg_units.size(1)) * args.data.block_size]
            seg_output *= mask[:, start_frame * args.data.block_size : start_frame * args.data.block_size + seg_output.size(1)]
