...
class F0Extractor:
    def __init__(self, f0_extractor, sample_rate = 44100, hop_size = 512, f0_min = 65, f0_max = 800):
        ...
        if f0_extractor == 'crepe':
            ...
            if key_str not in RESAMPLE_KERNEL:
                RESAMPLE_KERNEL[key_str] = Resample(sample_rate, 16000, lowpass_filter_width = 128)
            self.resample_kernel = RESAMPLE_KERNEL[key_str]
        ...
        if f0_extractor == 'obphind':
            self.device_obphind = 'cuda' if torch.cuda.is_available() else 'cpu'
            key_str = str(sample_rate)
            if key_str not in RESAMPLE_KERNEL:
                RESAMPLE_KERNEL[key_str] = Resample(sample_rate, 16000, lowpass_filter_width = 128)
            self.resample_kernel = RESAMPLE_KERNEL[key_str]
            if 'obphind' not in F0_KERNEL:
                from modules.encoders.obphind import load_model as load_obphind_model
                F0_KERNEL['obphind'] = load_obphind_model('models/pretrained/obphind/model.pt', device = self.device_obphind)
            self.obphind, self.obphind_args = F0_KERNEL['obphind']
    ...
    def extract(self, audio, uv_interp = False, device = None, silence_front = 0): # audio: 1d numpy array
        ...
        # extract f0 using obphind
        elif self.f0_extractor == "obphind":
            if device is None:
                device = 'cuda' if torch.cuda.is_available() else 'cpu'
            resample_kernel = self.resample_kernel.to(device)
            wav16k_torch = resample_kernel(torch.FloatTensor(audio).unsqueeze(0).to(device))
            
            audio_sign = wav16k_torch.squeeze().sign()
            if audio_sign.shape[0] % self.obphind.block_size != 0:
                audio_sign = F.pad(audio_sign, (0, self.obphind.block_size - (audio_sign.shape[0] % self.obphind.block_size)), mode="constant")
            audio_sign = audio_sign.view(-1, self.obphind.block_size).unsqueeze(0)
            
            f0 = self.obphind(audio_sign)
