...
from modules.loss import discriminator_loss, feature_loss, generator_loss, autocorr_loss
...
def train(args, initial_global_step, nets_g, nets_d, loader_train, loader_test):
    ...
    if args.train.ft_unit2ctrl:
        train_params += [f'{ddsp_model_prefix}unit2ctrl.']
    ...
    model, optimizer, scheduler, model_d, optimizer_d, scheduler_d, loss_func, loader_train, loader_test = accelerator.prepare(
        ...
        device_placement=[True, True, False, True, True, False, True, False, False],
    )
    
    scheduler.requires_grad_ = False
    ...
    if not state_loaded:
        state_loaded = saver.load_from_pretrained(
            model, optimizer, model_d, optimizer_d)
        if not state_loaded:
    ...
    enable_noise_injection = True   # add noise to inputs during training
        ...
        for batch_idx, data in enumerate(loader_train):
            ...
            with accelerator.autocast():
                if enable_noise_injection:
                    noise_gains = torch.rand(units.shape[0], 2).to(accelerator.device)
                    noise_units = torch.randn_like(units) * 0.501 * noise_gains[:, 0:1, None]  # max S/N ratio of about 6 dB
                    # noise_f0 = torch.randn_like(f0) * 5. * noise_gains[:, 1:2, None]    # max f0 deviation of about +/- 5 Hz
                    # signal = model(units.to(dtype) + noise_units, f0 + noise_f0, volume, data[spk_id_key])
                    signal = model(units.to(dtype) + noise_units, f0, volume, data[spk_id_key], infer=False)
                else:
                    signal = model(units.to(dtype), f0, volume, data[spk_id_key], infer=False)
            ...
            if model_d is not None:
                ...
                # with accelerator.autocast():
                #     losses.append(autocorr_loss(signal, audio, hop_length=args.data.block_size//2)*.01)
