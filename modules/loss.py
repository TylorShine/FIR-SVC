...
class LF4SLoss(nn.Module):
    ...
    def __init__(self, n_fft=111, alpha=1.0, overlap=0, eps=1e-7, window_fn=torch.hann_window, wkwargs=None, log_freq_clip_min=16):
        ...
        self.spec = torchaudio.transforms.Spectrogram(
            ...
            # normalized=False, center=True, pad_mode='constant', window_fn=window_fn, wkwargs=wkwargs)
    ...
    def forward(self, x_true, x_pred):
        ...
        log_term = F.l1_loss(torch.clamp(S_true.abs(), min=1e-12).log()*torch.sign(S_true), torch.clamp(S_pred.abs(), min=1e-12).log()*torch.sign(S_pred))

        loss = self.alpha * log_term
        loss = loss + converge_term
        return loss
    ...
    def log_frequency_scale(self, n_fft, min_clip_bins=16):
        ...
        # ret = torch.log2(torch.max(torch.tensor(min_clip_bins), torch.arange(0, n_fft//2+1)) + 2) / torch.log2(torch.tensor(n_fft + 2))
        ret[0] = 1.
        return ret
...
class LF4SMPLoss(nn.Module):
    ...
    def __init__(self, n_fft=111, alpha=1.0, beta=0.5, overlap=0, eps=1e-7, window_fn=torch.hann_window, wkwargs=None, log_freq_clip_min=16):
        super().__init__()
        self.n_fft = n_fft
        self.alpha = alpha
        self.beta = beta
        ...
        self.log_freq_scale = self.log_frequency_scale(n_fft, log_freq_clip_min)[None, :, None]
...
class DLFSSLoss(nn.Module):
    ...
    def __init__(self, fft_min, fft_max, alpha=1.0, beta=0.5, overlap=0, eps=1e-7, device='cuda'):
        ...
        self.lossdict[fft_min] = LF4SLoss(fft_min, alpha, overlap, eps, window_fn=torch.hann_window, log_freq_clip_min=12).to(device)
        self.lossdict[fft_min-1] = LF4SLoss(fft_min-1, alpha, overlap, eps, window_fn=torch.hann_window, log_freq_clip_min=12).to(device)
...
class MLFSSLoss(nn.Module):
    ...
    def __init__(self, n_ffts=[32, 64, 128, 256, 512, 1024, 2048], alpha=1.0, beta=0.5, overlap=0, eps=1e-7, device='cuda'):
        ...
        self.lossdict = {}
        self.n_ffts = n_ffts
        self.n_fft_max = max(n_ffts)
        self.n_fft_norm = 1./sum([n/self.n_fft_max for n in n_ffts])
        for n in n_ffts:
    ...
    def forward(self, x_pred, x_true):
        ...
        # time domain sample loss
        # value = F.l1_loss(x_true, x_pred)*0.3 + F.mse_loss(x_true, x_pred)*0.2
        # value = F.mse_loss(x_true, x_pred)*0.001
        value = F.l1_loss(x_true, x_pred)
        
        # for n in self.n_ffts:
        #     value += self.lossdict[n](x_true, x_pred)*self.beta*(self.n_fft_max)/len(self.n_ffts)
        for n in self.n_ffts:
            value += self.lossdict[n](x_true, x_pred)*self.beta*(n/self.n_fft_max)*self.n_fft_norm
...
class MSSLoss(nn.Module):
    ...
    def __init__(self, n_ffts=[32, 64, 128, 256, 512, 1024, 2048], alpha=1.0, beta=0.5, overlap=0, eps=1e-7, device='cuda'):
        ...
        for n in n_ffts:
            # max_window_width = n_fft/n
            # max_overwrap = overlap + (1.-overlap)*(1.-1./max_window_width)
            self.lossdict[n] = SSSLoss(n, alpha, overlap, eps, window_fn=torch.hann_window).to(device)
...
class MLFSSMPLoss(nn.Module):
    '''
    Multiple Log-Frequency Scaled Spectral Magnitude and Phase Loss
    '''
    
    def __init__(self, n_ffts=[32, 64, 128, 256, 512, 1024, 2048], alpha=1.0, beta=0.5, gamma=0.02, overlap=0, eps=1e-7, device='cuda'):
        super().__init__()
        self.beta = beta
        self.lossdict = {}
        self.n_ffts = n_ffts
        ...
        for n in n_ffts:
            # max_window_width = n_fft/n
            # max_overwrap = overlap + (1.-overlap)*(1.-1./max_window_width)
            self.lossdict[n] = LF4SMPLoss(n, alpha, gamma, overlap, eps, window_fn=torch.hann_window, log_freq_clip_min=12).to(device)
...
class MultiScaleMelSpectrogramLoss(nn.Module):
            ...
            self.stft_params = [
                {
                    ...
                    # "window_fn": window_fn,
                    # "win_kwargs": {"width": ww},
                    # "log_freq_scale": self.log_frequency_scale(w)[None, :, None],
                }
    ...
    # @staticmethod
    # def log_frequency_scale(n_fft, min_clip_bins=16):
    #     ret = torch.log2(torch.max(torch.tensor(min_clip_bins), torch.arange(0, n_fft//2+1)) + 2) / torch.log2(torch.tensor(n_fft//2+1 + 2))
    #     ret[0] = 1.
    #     return ret
    ...
    # def to(self, device=None, dtype=None, non_blocking=False, memory_format=torch.preserve_format):
    #     super().to(device=device, dtype=dtype, non_blocking=non_blocking, memory_format=memory_format)
    #     for i in range(len(self.stft_params)):
    #         self.stft_params[i]["log_freq_scale"] = self.stft_params[i]["log_freq_scale"].to(device=device, dtype=dtype, non_blocking=non_blocking, memory_format=memory_format)
    #         # self.log_freq_scale = self.log_freq_scale.to(device=device, dtype=dtype, non_blocking=non_blocking, memory_format=memory_format)
    #     return self
    ...
    def mel_spectrogram(
        ...
        # magnitude = stft.abs() * log_freq_scale[..., None]
        magnitude = stft.abs()
        angle = stft.angle()
...
def autocorr_loss(x, y, hop_length=512):
    # return F.l1_loss(F.conv1d(x[:, None, :], x[:, None, :]), F.conv1d(y[:, None, :], y[:, None, :]))
    x = x.view(x.shape[0], hop_length, -1)
    y = y.view(y.shape[0], hop_length, -1)
    return F.l1_loss(F.conv1d(x, x), F.conv1d(y, y))
